...
actor class Root(init_args : ?T.RootInitArg) = this {
  ...
  stable let _init = switch (init_args) {
    case (?a) { a };
    case (null) { Debug.trap("No args provided") };
  };

  let NTN_GOV = Principal.fromText("eqsml-lyaaa-aaaaq-aacdq-cai");

  let DVECTOR_CYCLES = 40_000_000_000_000;
  ...
  public shared ({ caller }) func add_pair(iargs : T.ProdInitArg) : async () {
    ...
    let full_args : T.InitArg = {
      iargs with DEFI_AGGREGATOR = _init.DEFI_AGGREGATOR;
